...
$result = query("SELECT * FROM account WHERE acct_no='$acct'");
 if(!empty($result)){
   ...
   $ac = $row['save_amt'];
   if($count == 5){
  echo "<label>Shares Amount:     #3000 Pay Shares <input type=radio value=shares id=shares /></label><label>Bank Teller(optional)</label><input type='text' class=form-control id=bank_teller name='bank_teller'/><label for=amt>Deposit Amount</label><input type=Number name=amt class=form-control id=amt value=$ac >";
   }else{
